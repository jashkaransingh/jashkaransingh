...
**Software Engineering Intern @ Rootchat** (NYC, summer 2024)
rebuilt the entire user onboarding from scratch, 14 screens in Swift UIKit, with haptics across the whole app so every interaction had feedback and push notifications wired end to end through APNs. then went down a layer and built the real-time messaging backend in FastAPI, WebSockets holding the live connections and Redis Pub/Sub underneath so a message fans out to every subscribed connection instead of getting trapped in the process that received it. made the PostgreSQL-backed endpoints idempotent with client-generated UUIDs, because a phone on a bad subway connection retries, and a retry that writes twice is a duplicate message sitting in someone's chat forever. swapped the SQL polling that tracked who was online for Redis TTL heartbeats, so presence expires on its own when a client goes quiet and the database stops answering the same question every few seconds. the part that taught me the most was the room, the founder pulled me into investor meetings without warning and once handed me a live call with the tech team to take over and brief everyone the next day, small team, every decision mattered, and I got used to being thrown past my comfort zone until it stopped feeling like a stretch.
